...
describe('getProductList', () => {
  ...
  it('should return a list of products', async () => {
    jest.spyOn(productService, 'getProductsFromDB').mockImplementation(() => Promise.resolve(getProductsListMock));

    const result = (await getProductsList(null, null, null)) as APIGatewayProxyResult;
    expect(result.statusCode).toBe(200);
    expect(JSON.parse(result.body)).toEqual(getProductsListMock);
  })

  it('should return 500 if getting products failed', async () => {
    jest.spyOn(productService, 'getProductsFromDB').mockReturnValueOnce(null);
